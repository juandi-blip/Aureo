...
async function loadDatabase() {
    ...
    // Migración: completar código postal de proveedores sembrados antes de este campo
    if (typeof DEMO_DATA !== 'undefined' && (state.suppliers || []).some(s => !s.postal)) {
        if (!_demoDE) _demoDE = DEMO_DATA.buildDE(new Date());
        const _postalById = {};
        _demoDE.suppliers.forEach(s => { _postalById[s.id] = s.postal; });
        let _postalMigrated = false;
        state.suppliers.forEach(s => {
            if (!s.postal && _postalById[s.id]) { s.postal = _postalById[s.id]; _postalMigrated = true; }
        });
        if (_postalMigrated) saveDEKey('suppliers');
    }
...
function switchTab(tabId) {
    ...
    switch (tabId) {
        ...
        case 'inventario':
            ...
            break;
        case 'compras':
            headerTitle.innerText = "Compras a Proveedores";
            headerDesc.innerText = "Solicitudes de pedido, generación de órdenes de compra y consulta por proveedor.";
            renderCompras();
            break;
        case 'dataentry':
...
// --- Categorías de materiales (lista global extensible; persiste en localStorage) ---
const DE_MAT_CATEGORIES_BASE = ['Herramientas Eléctricas', 'Herramientas Manuales', 'Consumibles y EPP', 'Fijaciones y Anclajes', 'Maquinaria Pesada', 'Materiales de Construcción', 'Pinturas y Acabados', 'Eléctrico y Cableado', 'Plomería', 'Otros'];

function getMatCategories() {
    let cats = null;
    try { cats = JSON.parse(localStorage.getItem('vf_de_matCategories') || 'null'); } catch { cats = null; }
    if (!Array.isArray(cats) || cats.length === 0) cats = [...DE_MAT_CATEGORIES_BASE];
    // Integrar categorías ya presentes en los materiales registrados
...
function addMatCategory(name) {
    if (!name) return;
    const cats = getMatCategories();
    if (!cats.includes(name)) cats.push(name);
    localStorage.setItem('vf_de_matCategories', JSON.stringify(cats));
}

function onDE_MatCategoryChange(sel) {
    const inp = document.getElementById('de-mat-category-new');
    if (!inp) return;
        ...
        <form onsubmit="saveDE_Material(event)">
          ...
          <div class="form-row" style="grid-template-columns: 1fr 1fr 1fr;">
            ...
            <div class="form-group">
              ...
              <select class="form-select" id="de-mat-category" onchange="onDE_MatCategoryChange(this)">
                ${getMatCategories()
            .map(c => `<option${isEdit && editing.category === c ? ' selected' : ''}>${c}</option>`).join('')}
                <option value="__new__">➕ Crear nueva categoría…</option>
              </select>
              <input type="text" class="form-input" id="de-mat-category-new"
                placeholder="Nombre de la nueva categoría" maxlength="40"
...
function saveDE_Material(e) {
    ...
    let category = document.getElementById('de-mat-category').value;
    if (category === '__new__') {
        category = (document.getElementById('de-mat-category-new').value || '').trim();
        if (!category) {
            triggerToast('error', 'Escribe el nombre de la nueva categoría.');
            return;
        }
...
function confirmDE_MatImport() {
    ...
    _deMatImportRows.forEach(row => {
        ...
        const category = String(row.category || 'Otros').trim() || 'Otros';
        addMatCategory(category); // las categorías del Excel se integran a la lista global
...
// --------------------------------------------------------------------------
// --- Códigos postales de Colombia (sistema 4-72: 6 dígitos, prefijo por departamento) ---
const CO_POSTAL_CODES = [
    { code: '110111', city: 'Bogotá D.C.' },
    { code: '050001', city: 'Medellín' },
    { code: '760001', city: 'Cali' },
    { code: '080001', city: 'Barranquilla' },
    { code: '130001', city: 'Cartagena de Indias' },
    { code: '680001', city: 'Bucaramanga' },
    { code: '540001', city: 'Cúcuta' },
    { code: '660001', city: 'Pereira' },
    { code: '170001', city: 'Manizales' },
    { code: '470001', city: 'Santa Marta' },
    { code: '200001', city: 'Valledupar' },
    { code: '230001', city: 'Montería' },
    { code: '730001', city: 'Ibagué' },
    { code: '500001', city: 'Villavicencio' },
    { code: '410001', city: 'Neiva' },
    { code: '520001', city: 'Pasto' },
    { code: '630001', city: 'Armenia' },
    { code: '190001', city: 'Popayán' },
    { code: '700001', city: 'Sincelejo' },
    { code: '150001', city: 'Tunja' },
    { code: '440001', city: 'Riohacha' },
    { code: '850001', city: 'Yopal' },
    { code: '270001', city: 'Quibdó' },
    { code: '810001', city: 'Arauca' },
    { code: '880001', city: 'San Andrés' },
    { code: '910001', city: 'Leticia' }
];

function coPostalCity(code) {
    const hit = CO_POSTAL_CODES.find(c => c.code === String(code || '').trim());
    return hit ? hit.city : '';
}
    ...
    <div class="dataentry-panel">
      ...
      <div class="dataentry-section-card">
        ...
        <form onsubmit="saveDE_Proveedor(event)">
          ...
          <div class="form-row" style="grid-template-columns: 140px 1fr 1fr 220px;">
            <div class="form-group">
              <label class="form-label">ID</label>
            ...
            <div class="form-group">
              <label class="form-label">Código Postal <span style="font-size:.78rem;color:var(--text-muted);">(municipio)</span></label>
              <input type="text" class="form-input" id="de-prov-postal" list="de-prov-postal-list"
                placeholder="Ej: 080001" pattern="\\d{6}" maxlength="6" autocomplete="off"
                title="Código postal colombiano de 6 dígitos"
                value="${isEdit ? (editing.postal || '') : ''}"
                oninput="onDE_ProvPostalInput(this)">
              <datalist id="de-prov-postal-list">
                ${CO_POSTAL_CODES.map(c => `<option value="${c.code}">${c.city}</option>`).join('')}
              </datalist>
              <span id="de-prov-postal-city" style="font-size:.78rem;color:var(--text-muted);margin-top:.3rem;display:block;">
                ${isEdit && editing.postal ? coPostalCity(editing.postal) : ''}
              </span>
            </div>
          </div>
          <div style="display:flex;justify-content:flex-end;gap:.75rem;margin-top:.5rem;">
      ...
      <div class="dataentry-section-card">
        ...
        <p style="font-size:.85rem;color:var(--text-muted);margin-bottom:1.25rem;">
          El archivo debe tener las columnas: <strong style="color:var(--text-secondary);">Proveedor · Acreedor · CodigoPostal</strong> (primera fila = encabezados; código postal Colombia de 6 dígitos, opcional).
        </p>
      ...
      <div class="dataentry-section-card">
        ...
        <div class="input-wrapper" style="margin-bottom:1.25rem;">
          ...
          <input type="text" class="form-input" id="de-prov-search"
            placeholder="Buscar por ID, proveedor, acreedor, código postal o municipio..."
            value="${_deProvSearch}"
...
function buildDE_ProvRows(list) {
    if (list.length === 0) {
        return `<tr><td colspan="5" style="text-align:center;color:var(--text-muted);padding:2.5rem 0;">Sin proveedores registrados.</td></tr>`;
    }
    return list.map(s => `
    <tr>
      ...
      <td style="white-space:nowrap;">
        ${s.postal ? `<span style="font-family:'JetBrains Mono',monospace;font-size:.85rem;font-weight:600;">${s.postal}</span>
        <span style="font-size:.78rem;color:var(--text-muted);"> ${coPostalCity(s.postal) || ''}</span>` : '—'}
      </td>
      <td style="text-align:center;">
        <div style="display:inline-flex;gap:.5rem;">
...
function filterDE_Prov(query) {
    ...
    const filtered = _deProvSearch
        ? state.suppliers.filter(s =>
            ...
            (s.postal || '').includes(_deProvSearch) ||
            coPostalCity(s.postal).toLowerCase().includes(_deProvSearch))
        : state.suppliers;
...
function onDE_ProvPostalInput(inp) {
    const cityEl = document.getElementById('de-prov-postal-city');
    if (cityEl) cityEl.textContent = coPostalCity(inp.value) || (inp.value.length === 6 ? 'Municipio no catalogado' : '');
}

function saveDE_Proveedor(e) {
    e.preventDefault();
    const editId = document.getElementById('de-prov-edit-id').value;
    const postal = document.getElementById('de-prov-postal').value.trim();
    if (postal && !/^\d{6}$/.test(postal)) {
        triggerToast('error', 'El código postal debe tener 6 dígitos (formato Colombia, ej: 080001).');
        return;
    }
    const prov = {
    ...
    if (editId) {
        ...
        triggerToast('success', `Proveedor "${prov.name}" actualizado.`);
        _deProvEditId = null;
    } else {
        state.suppliers.push(prov);
        triggerToast('success', `Proveedor "${prov.name}" registrado.`);
    }
    saveDEKey('suppliers');
...
const _DE_PROV_COL_MAP = {
    ...
    codigopostal: 'postal', códigopostal: 'postal', postal: 'postal', cp: 'postal', codpostal: 'postal', zipcode: 'postal', zip: 'postal'
};
...
function _renderDE_ProvImportPreview(filename) {
    ...
    <div class="de-import-preview-box">
      ...
      <div class="table-responsive" style="margin-top:.75rem;">
        <table class="custom-table">
          <thead><tr><th>Proveedor</th><th>Acreedor</th><th>C. Postal</th></tr></thead>
          <tbody>
            ${preview.map(r => `
            <tr>
              <td>${r.name || '—'}</td>
              <td style="font-family:'JetBrains Mono',monospace;font-size:.82rem;font-weight:600;">${r.acreedor || '—'}</td>
              <td style="font-family:'JetBrains Mono',monospace;font-size:.82rem;">${r.postal || '—'}${coPostalCity(r.postal) ? ` <span style="color:var(--text-muted);font-size:.76rem;">${coPostalCity(r.postal)}</span>` : ''}</td>
            </tr>`).join('')}
            ${more > 0 ? `<tr><td colspan="3" style="text-align:center;color:var(--text-muted);font-size:.8rem;">... y ${more} fila(s) más</td></tr>` : ''}
          </tbody>
        </table>
      </div>
      <div style="display:flex;justify-content:flex-end;gap:.75rem;margin-top:1rem;">
...
function confirmDE_ProvImport() {
    ...
    _deProvImportRows.forEach((row, i) => {
        ...
        const postal = /^\d{6}$/.test(String(row.postal || '').trim()) ? String(row.postal).trim() : '';
...
function downloadDE_ProvTemplate() {
    const ws = XLSX.utils.aoa_to_sheet([
        ['Proveedor', 'Acreedor', 'CodigoPostal'],
        ['Ferrox Andina Ltda.', 'ACR-0001', '110111'],
        ['TitanPro Abrasivos S.A.', 'ACR-0002', '050001']
    ]);
            ...
            const prodsHtml = catProds.map(function (p) {
                ...
                return `<div style="background:rgba(36,31,26,.04);border-radius:8px;padding:.75rem 1rem;margin:.3rem 0;border-left:3px solid ${abcClr};">
                    <div style="display:grid;grid-template-columns:90px 1fr 90px 1fr;gap:.3rem .6rem;font-size:.78rem;align-items:start;">
                        ...
                        <span style="font-weight:600;color:var(--text-primary);grid-column:2/5;">${p.name}</span>
...
function applyOneRelocation(productId, targetAisle) {
    if (_applyRelocationToProduct(productId, targetAisle)) {
        saveProductsToStorage();
        const product = state.products.find(p => p.id === productId);
        triggerToast('success', `${product?.name?.split(' ').slice(0, 3).join(' ')} → Pasillo ${targetAisle}.`);
        renderABCView();
    }
}
                ...
                <td colspan="8" style="padding:.75rem 1rem 1rem;">
                    <div style="background:var(--bg-card,#1a1a30);border:1px solid var(--accent-cyan,#2E4A6E);border-radius:10px;padding:1rem 1.25rem;">
                        <div style="font-size:0.78rem;font-weight:700;text-transform:uppercase;letter-spacing:.05em;color:var(--accent-cyan,#2E4A6E);margin-bottom:.9rem;">
                            Nueva ubicación para: <em style="color:var(--text-primary);">${p.name}</em>
                        </div>
...
function saveManualRelocation(productId) {
    ...
    saveProductsToStorage();
    _wmsEditId = null;
    triggerToast('success', `${product.name.split(' ').slice(0, 3).join(' ')} → ${newWarehouse}, Pasillo ${newAisle}, Est. ${newShelf}`);
    renderABCView();
}
    ...
    c.innerHTML = `
        ...
        <div class="card" style="margin-bottom:1.5rem;">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr 1fr auto;gap:1rem;margin-bottom:1.2rem;align-items:end;">
                ...
                <div class="form-group" style="margin:0;">
                    <label class="form-label">ID / RUT Cliente</label>
                    <input type="text" class="form-input" id="np-client-id" placeholder="RUT-XX.XXX.XXX-X">
                </div>
                <div class="form-group" style="margin:0;">
...
function addItemToNewPick() {
    ...
    triggerToast('success', `${product.name} agregado (${qty} uds).`);
}
...
function exportAllPickingPDF() {
    ...
    const now = new Date();
    const printDate = now.toLocaleDateString('es-CL') + ' ' + now.toLocaleTimeString('es-CL', { hour: '2-digit', minute: '2-digit' });
...
<div class="meta">
  <div class="meta-col">
    ...
    <div class="meta-row"><span class="mk">Cliente</span><span class="mv">${list.clientName}</span></div>
